...
async function main() {
  ...
  const escrowPath = path.join(__dirname, '../soroban-escrow');
  execSync('cargo build --target wasm32-unknown-unknown --release', { cwd: escrowPath });
  const wasm = fs.readFileSync(path.join(escrowPath, 'target/wasm32-unknown-unknown/release/soroban_escrow.wasm'));
  ...
  let tx5 = new TransactionBuilder(oracleAccRpc2, { fee: BASE_FEE, networkPassphrase: Networks.TESTNET })
    .addOperation(Operation.createCustomContract({
      wasmId,
      address: new Contract(oracleKp.publicKey()),
      constructorArgs: [
