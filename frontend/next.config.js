...
const nextConfig = {
  ...
  async rewrites() {
    ...
    // afterFiles: Next.js primeiro verifica rotas locais (app/api/*) e só então
    // aplica estes rewrites. Isso garante que /api/locations/search seja servido
    // pelo Route Handler local sem precisar de nenhuma regra especial.
    return {
      afterFiles: [
        {
          source: '/api/:path*',
